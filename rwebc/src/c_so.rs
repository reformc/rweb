...
mod tests {
    ...
    #[test]
    fn test_quic_node_run() {
        let proxy_list = r#"[{"mac":"aabbccddeeff","url":"http://192.168.199.1"}]"#;
        let proxy_list = serde_json::from_str::<Vec<proxy_list::ProxyList>>(proxy_list).unwrap();
        println!("proxy_list: {:?}", proxy_list);
    }
}